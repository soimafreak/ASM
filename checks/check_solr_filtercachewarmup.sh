...
then
  ...
  then
    ...
    then
      echo -e "CRITICAL :: FilterCache warmup is $warmup secs|'warmup'=$warmup;$warning;$critical;;"
      exit 2
    else
      echo -e "WARNING :: FilterCache warmup is $warmup secs|'warmup'=$warmup;$warning;$critical;;"
      exit 1
    fi
  else
    echo -e "OK :: FilterCache warmup is $warmup secs|'warmup'=$warmup;$warning;$critical;;"
    exit 0
  fi
else
  echo -e "UNKNOWN :: FilterCache warmup is $warmup secs"
  exit 3
