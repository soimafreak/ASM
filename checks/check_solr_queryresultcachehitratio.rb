...
hitratio=solr_results.get_queryResultCache_hitratio("alfresco").to_f
lookups=solr_results.get_queryResultCache_lookups("alfresco").to_i
...
if (inverse.is_a? Float)
  if ( lookups >= 100 )
    if ( inverse >= warning )
      if (inverse >= critical )
        puts "CRITICAL :: QueryResultCache hitratio is #{hitratio}|'hitratio'=#{hitratio};#{warning};#{critical};;"
        exit 2
      else
        puts "WARNING :: QueryResultCache hitratio is #{hitratio}|'hitratio'=#{hitratio};#{warning};#{critical};;"
        exit 1
      end
    else
      puts "OK :: QueryResultCache hitratio is #{hitratio}|'hitratio'=#{hitratio};#{warning};#{critical};;"
      exit 0
    end
  else
    puts "OK :: QueryResultCache hitratio is #{hitratio}|'hitratio'=#{hitratio};#{warning};#{critical};;"
  ...
  puts "UNKNOWN :: QueryResultCache hitratio is #{hitratio}"
  exit 3
end
