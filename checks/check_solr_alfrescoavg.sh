...
critical=2000
warning=1000
avg=`/usr/local/nagios/libexec/nrpe_local/check_solr alfresco alfrescoavg localhost`
...
then
  ...
  then
    if [ $avg -ge $critical ]
    then
      echo -e "CRITICAL :: Alfresco AVG is $avg msecs|'avg'=$avg;$warning;$critical;;"
      exit 2
    else
      echo -e "WARNING :: Alfresco AVG is $avg msecs|'avg'=$avg;$warning;$critical;;"
      exit 1
    fi
  else
    echo -e "OK :: Alfresco AVG is $avg msecs|'avg'=$avg;$warning;$critical;;"
    exit 0
  fi
else
  echo -e "UNKNOWN :: Alfresco AVG is $avg msecs"
  exit 3
