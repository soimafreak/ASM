...
hitratio=solr_results.get_alfrescoAuthorityCache_hitratio("alfresco").to_f
lookups=solr_results.get_alfrescoAuthorityCache_lookups("alfresco").to_i
...
if (inverse.is_a? Float)
  if ( lookups >= 100 )
    if ( inverse >= warning )
      if (inverse >= critical )
        puts "CRITICAL :: AlfrescoAuthorityCache hitratio is #{hitratio}|'hitratio'=#{hitratio};#{warning};#{critical};;"
        exit 2
      else
        puts "WARNING :: AlfrescoAuthorityCache hitratio is #{hitratio}|'hitratio'=#{hitratio};#{warning};#{critical};;"
        exit 1
      end
    else
      puts "OK :: AlfrescoAuthorityCache hitratio is #{hitratio}|'hitratio'=#{hitratio};#{warning};#{critical};;"
      exit 0
    end
  else
    puts "OK :: AlfrescoAuthorityCache hitratio is #{hitratio}|'hitratio'=#{hitratio};#{warning};#{critical};;"
  ...
  puts "UNKNOWN :: AlfrescoAuthorityCache hitratio is #{hitratio}"
  exit 3
end
