...
cumulative_hitratio=solr_results.get_alfrescoPathCache_cumulative_hitratio("alfresco").to_f
cumulative_lookups=solr_results.get_alfrescoPathCache_lookups("alfresco").to_i
...
if (inverse.is_a? Float)
  if ( cumulative_lookups >= 100 )
    if ( inverse >= warning )
      if (inverse >= critical )
        puts "CRITICAL :: AlfrescoPathCache cumulative_hitratio is #{cumulative_hitratio}|'cumulative_hitratio'=#{cumulative_hitratio};#{warning};#{critical};;"
        exit 2
      else
        puts "WARNING :: AlfrescoPathCache cumulative_hitratio is #{cumulative_hitratio}|'cumulative_hitratio'=#{cumulative_hitratio};#{warning};#{critical};;"
        exit 1
      end
    else
      puts "OK :: AlfrescoPathCache cumulative_hitratio is #{cumulative_hitratio}|'cumulative_hitratio'=#{cumulative_hitratio};#{warning};#{critical};;"
      exit 0
    end
  else
    puts "OK :: AlfrescoPathCache cumulative_hitratio is #{cumulative_hitratio}|'cumulative_hitratio'=#{cumulative_hitratio};#{warning};#{critical};;"
  ...
  puts "UNKNOWN :: AlfrescoPathCache cumulative_hitratio is #{cumulative_hitratio}"
  exit 3
end
