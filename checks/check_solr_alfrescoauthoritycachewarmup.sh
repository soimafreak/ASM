...
warmup=`/usr/local/nagios/libexec/nrpe_local/check_solr alfresco alfrescoauthoritycachewarmup localhost`
critical=60000
warning=30000
...
then
  ...
  then
    ...
    then
      echo -e "CRITICAL :: AlfrescoAuthorityCache warmup is $warmup msecs|'warmup'=$warmup;$warning;$critical;;"
      exit 2
    else
      echo -e "WARNING :: AlfrescoAuthorityCache warmup is $warmup msecs|'warmup'=$warmup;$warning;$critical;;"
      exit 1
    fi
  else
    echo -e "OK :: AlfrescoAuthorityCache warmup is $warmup msecs|'warmup'=$warmup;$warning;$critical;;"
    exit 0
  fi
else
  echo -e "UNKNOWN :: AlfrescoAuthorityCache warmup is $warmup msecs"
  exit 3
