...
critical=900
warning=600
lag=`/usr/local/nagios/libexec/nrpe_local/check_solr archive lag localhost`
...
then
  ...
  then
    if [ $lag -ge $critical ]
    then
      echo -e "CRITICAL :: Lag on archive is $lag secs|'lag'=$lag;$warning;$critical;;"
      exit 2
    else
      echo -e "WARNING :: Lag on archive is $lag secs|'lag'=$lag;$warning;$critical;;"
      exit 1
    fi
  else
    echo -e "OK :: Lag on archive is $lag secs|'lag'=$lag;$warning;$critical;;"
    exit 0
  fi
else
  echo -e "UNKNOWN :: Lag on archive is $lag secs"
  exit 3
fi
exit $?
