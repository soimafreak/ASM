...
if (inverse.is_a? Float)
  if ( cumulative_lookups >= 100 )
    if ( inverse >= warning )
      if (inverse >= critical )
        puts "CRITICAL :: QueryResultCache cumulative_hitratio is #{cumulative_hitratio}|'cumulative_hitratio'=#{cumulative_hitratio};#{warning};#{critical};;"
        exit 2
      else
        puts "WARNING :: QueryResultCache cumulative_hitratio is #{cumulative_hitratio}|'cumulative_hitratio'=#{cumulative_hitratio};#{warning};#{critical};;"
        exit 1
      end
    else
      puts "OK :: QueryResultCache cumulative_hitratio is #{cumulative_hitratio}|'cumulative_hitratio'=#{cumulative_hitratio};#{warning};#{critical};;"
      exit 0
    end
  else
    puts "OK :: QueryResultCache cumulative_hitratio is #{cumulative_hitratio}|'cumulative_hitratio'=#{cumulative_hitratio};#{warning};#{critical};;"
  ...
  puts "UNKNOWN :: QueryResultCache cumulative_hitratio is #{cumulative_hitratio}"
  exit 3
end
