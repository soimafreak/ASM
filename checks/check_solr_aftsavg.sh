...
then
  ...
  then
    if [ $avg -ge $critical ]
    then
      echo -e "CRITICAL :: AFTS AVG is $avg msecs|'avg'=$avg;$warning;$critical;;"
      exit 2
    else
      echo -e "WARNING :: AFTS AVG is $avg msecs|'avg'=$avg;$warning;$critical;;"
      exit 1
    fi
  else
    echo -e "OK :: AFTS AVG is $avg msecs|'avg'=$avg;$warning;$critical;;"
    exit 0
  fi
else
  echo -e "UNKNOWN :: AFTS AVG is $avg msecs"
  exit 3
